...
const defaultFeatures = [
	{
		id: 1,
		name: "High-Quality Kitchen Cabinets & Wardrobes",
		icon: "",
	},
	{
		id: 2,
				...
				<div className="bg-white mt-4 space-y-4 p-6">
					...
					<div className=" space-y-4 max-w-6xl pt-4">
						...
						<div className="flex items-center justify-between">
							...
							<p
								className={`${
									rental.status === "RENTED" ? "text-[#13A017]"
									: rental.status === "AVAILABLE" ? "text-[#116114]"
									: "text-[#FF6B35]"
								}`}
							>
								{rental.status || "N/A"}
					...
					<div className="flex flex-wrap gap-4">
						{(property?.features || defaultFeatures)?.map(
							(feature: any, index: number) => (
								<IconDisplay key={feature.id} item={feature} />
					...
					<div className="flex  gap-4">
						{(property?.amenities || defaultAmenities)?.map(
							(amenity: any, index: number) => (
								<IconDisplay key={amenity.id} item={amenity} />
								// <div
								// 	key={index}
								// 	className="border border-gray-200 rounded-lg px-4 py-2 w-fit flex items-center h-auto"
								// >
