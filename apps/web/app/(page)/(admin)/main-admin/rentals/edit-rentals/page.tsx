...
export default function EditRental() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Validate that the number of units doesn't exceed available units
      if (formData.availableUnits > selectedUnit.availableUnits) {
        toast.error(
          `Cannot add ${formData.availableUnits} units. Only ${selectedUnit.availableUnits} units available for this apartment type.`
        );
        setIsSubmitting(false);
        return;
      }
  ...
  // Get max available units for the selected apartment type
  const selectedUnit = selectedProperty?.units?.find(
    (unit) => unit.unitType === formData.apartmentType
  );
  const maxAvailableUnits = selectedUnit?.availableUnits || 0;
      ...
      <form onSubmit={handleSubmit} className="space-y-6 mt-4 bg-white p-6">
        ...
        <div>
          ...
          <Dropdown
            ...
            onChange={(value) => {
              ...
              // Reset availableUnits when apartment type changes
              const newSelectedUnit = selectedProperty?.units?.find(
                (unit) => unit.unitType === value
              );
              if (
                newSelectedUnit &&
                formData.availableUnits > newSelectedUnit.availableUnits
              ) {
                handleInputChange("availableUnits", 0);
              }
            }}
            required
          />
        ...
        <div>
          <label className="block mb-1 text-sm text-[#323539] font-medium">
            ...
            {maxAvailableUnits > 0 && (
              <span className="text-xs text-[#858C95] ml-2">
                (Max: {maxAvailableUnits} available)
              </span>
            )}
          </label>
          <Input
            ...
            onChange={(e) => {
              ...
              handleInputChange("availableUnits", value);
            }}
            placeholder={
              maxAvailableUnits > 0
                ? `Max ${maxAvailableUnits}`
                : "Select apartment type first"
            }
            disabled={!formData.apartmentType}
            required
            className={`bg-[#E5E5E7] border ${
              formData.availableUnits > maxAvailableUnits &&
              maxAvailableUnits > 0
                ? "border-red-500"
                : "border-[#116114]"
            }`}
          ...
          {formData.availableUnits > maxAvailableUnits &&
            maxAvailableUnits > 0 && (
              <p className="text-red-500 text-sm mt-1">
                Cannot exceed {maxAvailableUnits} available units
              </p>
            )}
          {!formData.apartmentType && (
            <p className="text-[#858C95] text-xs mt-1">
              Please select an apartment type first
            </p>
          )}
        </div>
