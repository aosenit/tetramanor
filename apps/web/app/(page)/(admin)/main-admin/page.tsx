...
function useNotifications() {
  ...
  const { data, isLoading, isError } = useFetchData(
    "notifications?page=1&limit=5&sortOrder=desc&status=UNREAD"
  );

  useEffect(() => {
    if (data?.data?.items) setNotifications(data?.data?.items);
      ...
      {notifications?.map((n: any) => (
        <div key={n.id} className="flex items-center gap-3 py-2 px-1">
          <div className="flex-1">
            ...
            <div className="text-xs text-gray-500 md:w-[85%]">{n.message}</div>
          </div>
          {/* <input
            type="checkbox"
            checked={!!n.read}
